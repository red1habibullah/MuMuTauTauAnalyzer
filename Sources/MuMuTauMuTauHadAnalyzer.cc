   ...
   for (Long64_t jentry=0; jentry<nentries; jentry++) {
      ...
      for (unsigned int iTau=0; iTau<recoTauPt->size(); iTau++)
      {
          ...
          for (unsigned int iMuon=0; iMuon<recoMuonPt->size(); iMuon++)
          {
              ...
              if ((Tau.DeltaR(Mu3Cand) < smallestDR) && (recoTauPDGId->at(iTau)/fabs(recoTauPDGId->at(iTau)) == (-1) * recoMuonPDGId->at(iMuon)/fabs(recoMuonPDGId->at(iMuon))) && ((Tau+Mu3Cand).M() < 60.0))
              {
          ...
          for (unsigned int iMuon=0; iMuon<Mu1s.size(); iMuon++)
          {
              ...
              for (unsigned int iTau=0; iTau<Taus.size(); iTau++)
              {
                  ...
                  if (Mu1.DeltaR(Mu3) > 0.4 && Mu2.DeltaR(Mu3) > 0.4 && Mu1.DeltaR(Tau) > 0.8 && Mu2.DeltaR(Tau) > 0.8)
                  {
                      ...
                      mu3Iso->Fill(Mu3Iso.at(iTau), weight);
