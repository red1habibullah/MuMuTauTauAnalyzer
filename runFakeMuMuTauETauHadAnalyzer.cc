...
int main(int argc, char **argv)
{
    ...
    if (doWhat == "TTJETS" || doWhat == "ALL")
    {
        if (inputFile.EndsWith(".root"))
        {
            ...
            FakeMuMuTauETauHadAnalyzer TTJetsHist(inputFile, outputDir, lumi*831.76*1000, summedWeights, maxEvents, true);
            TTJetsHist.Loop();
        } // end if inputFile.EndsWith(".root")
            ...
            while (getline(finTree, fileName))
            {
                FakeMuMuTauETauHadAnalyzer TTJetsHist(fileName, outputDir, lumi*831.76*1000, summedWeights, maxEvents, true);
                TTJetsHist.Loop();
            } // end while loop on input file list
        } // end else
    } // end if TTJets
